class ErrorCode:
    """统一 4 位数业务错误码。"""

    # 0xxx — 成功
    SUCCESS = "0000"

    # 1xxx — 认证/授权
    AUTH_MISSING = "1001"
    AUTH_INVALID = "1002"

    # 2xxx — 玩家相关
    PLAYER_NOT_FOUND = "2001"
    PLAYER_NO_NUCLEUS_ID = "2002"
    PLAYER_NOT_ONLINE = "2003"

    # 3xxx — 服务器状态
    NO_ONLINE_SERVERS = "3003"

    # 4xxx — 资源未找到
    DONATION_NOT_FOUND = "4001"
    SERVER_NOT_FOUND = "4002"

    # 5xxx — 参数校验
    INVALID_REASON = "5001"
    INVALID_WEAPON = "5002"

    # 6xxx — 绑定相关
    BINDING_PLAYER_NOT_FOUND = "6001"
    BINDING_ALREADY_EXISTS = "6002"
    BINDING_NOT_FOUND = "6003"
    BINDING_PLAYER_AMBIGUOUS = "6004"

    # 7xxx — 组队相关
    TEAM_NOT_FOUND = "7001"
    TEAM_ALREADY_FULL = "7002"
    TEAM_ALREADY_IN_TEAM = "7003"
    TEAM_NOT_CREATOR = "7004"
    TEAM_INVALID_SLOTS = "7005"
    TEAM_CANNOT_JOIN_OWN = "7006"
    TEAM_NOT_MEMBER = "7007"

    # 8xxx — Apex Legends Status API
    APEX_NOT_CONFIGURED = "8001"
    APEX_API_ERROR = "8002"
    APEX_INVALID_REQUEST = "8003"

    # 9xxx — 游戏配置社区
    GAME_CONFIG_NOT_FOUND = "9001"
    GAME_CONFIG_UPLOAD_DISABLED = "9002"
    GAME_CONFIG_INVALID_CONTENT = "9003"
    GAME_CONFIG_CONTENT_TOO_LARGE = "9004"
    GAME_CONFIG_INVALID_SOURCE_GAME = "9005"
