...
_IPV4_WITH_PORT_RE = re.compile(r"^((?:\d{1,3}\.){3}\d{1,3}):(\d{1,5})$")
...
async def resolve_server(query: str) -> Server | None:
    ...
    address_match = _IPV4_WITH_PORT_RE.match(query)
    if address_match:
        host, port_text = address_match.groups()
        return await Server.filter(host=host, port=int(port_text)).order_by("-last_seen_at").first()

    if _IPV4_RE.match(query):
        return await Server.filter(host=query).order_by("-last_seen_at").first()
