...
async def get_player_vs_all_stats(
    ...
    return paginated(data=results, total=total, msg=f"KD Leaderboard for {nucleus_id_or_player_name}", **extra)
...
async def get_player_weapon_stats(
    ...
    if server_obj:
        extra["server"] = _server_info(server_obj)
    return paginated(data=results, total=total, msg=f"Weapon stats for {nucleus_id_or_player_name}", **extra)
