...
SORT_DISPLAY = {"kd": "KD", "kills": "击杀数", "deaths": "死亡数"}


def _weapon_cn(key: str) -> str:
    return WEAPON_CN.get(key.lower(), key)
...
async def handle_check_weapons(args: Message = CommandArg()) -> None:
    ...
    try:
        ...
        for w in display:
            ...
            weapon_name = _weapon_cn(weapon_key)
            kd = w.get("kd", 0)
            k = w.get("kills", 0)
            d = w.get("deaths", 0)
...
async def handle_weapon_leaderboard(args: Message = CommandArg()) -> None:
    ...
    try:
        ...
        resp = await api_client.get_weapon_leaderboard(
