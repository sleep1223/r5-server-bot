    ...
    try:
        ...
        if res.get("code") == "0000":
            ...
            if data.get("player_online") is True:
                ...
                server_name = primary.get("name") or (f"{primary.get('host', 'unknown')}:{primary.get('port', 'unknown')}")
                async_count = data.get("async_server_count", 0)
                await cmd_ban.finish(f"✅ 封禁成功\n\n🟢 玩家在线: 是\n🖥️ 服务器: {server_name}\n🔄 后台补执行: {async_count} 个服务器\n{res.get('msg')}")
...
async def handle_unban(args: Message = CommandArg()) -> None:
    ...
    try:
        ...
        traceback.print_exc()
        await cmd_unban.finish("⏳ 解封请求超时，服务器可能仍在后台执行，请稍后重试或查询状态。")
    except Exception as e:
        traceback.print_exc()
